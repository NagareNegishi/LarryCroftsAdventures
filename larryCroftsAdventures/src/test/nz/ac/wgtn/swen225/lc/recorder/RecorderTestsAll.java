...
public class RecorderTestsAll {
	...
	public void setup() {
		...
		mockGameStateSupplier = () -> new GameState(mockMazeSupplier.get(), mockChapSupplier.get(), 10, 0, Map.of(),
				time, mockAppNotifier, new ArrayList<>(), 1);
	...
	public void testRecordingWithLargeMaze() {
		Supplier<GameStateController> largeGameGiver = () -> new GameStateController(
				new GameState(Maze.createBasicMaze(1000, 1000), new Chap(1, 1, new ArrayList<>()), 10, 0, Map.of(),
						time, mockAppNotifier, new ArrayList<>(), 1));
	...
	public void testAutoReplayBackwards() {
		...
		for (int i = 0; i < 1000; i++) {
			...
			try {
				...
				if (gameModel.getChap().getCol() == 1 && gameModel.getChap().getRow() == 1)
					gameModel.moveChap(d);
	...
	public void testNullFirstLevelSupplier() {
		try {
			...
			if (ae.getMessage().equals("Null first level supplier given to record during construction!")) {
				return;
			}
		}
		assert false;
	...
	public void testsUsingNonTestingConstructor() {
		...
		for (int i = 0; i < 100; i++) {
			...
			try {
				...
				if (gameModel.getChap().getCol() == 9 && gameModel.getChap().getRow() == 13)
					gameModel.moveChap(d);
